...
import {
	...
	type Segment,
	useProjectEditor,
} from "@/app/(main)/create/layout";
import { MessageCard, TypingIndicator } from "@/components/ai/MessageCard";
import { type StreamEvent, streamChat } from "@/lib/ai/service";
...
export function ChatPanel() {
	...
	const toolCallsRef = useRef<
		Record<
			number,
			{ id: string; name: string; args: string; segmentIndex: number }
		>
	>({});
	...
	const supportsThinking =
		AI_MODELS[selectedModel === "deepseek-reasoner" ? "reasoner" : "chat"]
			.supportsThinking;
		...
		(name: string, argsJson: string): string => {
			try {
				...
				if (name.startsWith("replace_in_")) {
					const type = name.replace("replace_in_", "") as
						| "html"
						| "css"
						| "javascript";
					const contents = {
					...
					if (
						...
						if (count > 1)
							return JSON.stringify({
								success: false,
								error: `Found ${count} times. Must be unique.`,
							});
	...
	const updateAssistant = useCallback(
		(updater: (msg: Message) => Message) => {
			setMessages((prev) =>
				prev.map((m, i) => (i === currentMsgIndexRef.current ? updater(m) : m)),
			);
		},
		[setMessages],
	);
				...
				switch (event.type) {
					...
					case "content": {
						...
						updateAssistant((m) => {
							...
							if (currentTextSegmentRef.current < 0) {
								...
								segments[currentTextSegmentRef.current] = {
									type: "text",
									content: textContent,
								};
							}
					...
					case "tool_call_start": {
						...
						updateAssistant((m) => {
							...
							toolCallsRef.current[event.index] = {
								id: event.id,
								name: event.name,
								args: "",
								segmentIndex,
	...
	const finalizeToolCallsAndContinue = useCallback(
		async (textContent: string, reasoningContent: string) => {
			...
			const toolCallsArray = Object.keys(toolCalls)
				...
				.map((index) => ({
					...
					function: {
						...
						arguments: toolCalls[index].args,
					},
				}));
			...
			const { textContent: newText, reasoningContent: newReasoning } =
				await processStream();
	...
	return (
		<div className="flex flex-col h-full">
			...
			>
				...
				{isLoading && messages[messages.length - 1]?.segments.length === 0 && (
					<TypingIndicator />
				)}
	...
	return (
		<div className="flex flex-col items-center justify-center h-full text-center">
			...
			<p className="text-default-500 max-w-md">
				Describe what you want to build and I'll generate the HTML, CSS, and
				JavaScript for you.
			</p>
		</div>
	);
