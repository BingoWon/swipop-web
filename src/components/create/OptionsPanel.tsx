...
export function OptionsPanel() {
	...
	const [selectedAspect, setSelectedAspect] =
		useState<ThumbnailAspectRatio>("portrait");
	...
	const blobUrl = useMemo(
		() => (thumbnailBlob ? URL.createObjectURL(thumbnailBlob) : null),
		[thumbnailBlob],
	);
	useEffect(
		() => () => {
	...
	return (
		<div className="h-full overflow-auto p-4 space-y-6">
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								<div>
									...
									{saveError && (
										<p className="text-tiny text-danger">{saveError}</p>
									)}
								</div>
							</div>
							<Button
			...
			<Section title="Thumbnail" icon="solar:gallery-bold">
				<div className="space-y-4">
					...
					<div className="flex justify-center">
						<div
							...
							style={{
								...
								height:
									180 /
									(thumbnailAspectRatio || ASPECT_RATIOS[selectedAspect].ratio),
							}}
						>
							{thumbnailPreviewUrl ? (
								<img
					...
					<div className="flex justify-center gap-2">
						{(Object.keys(ASPECT_RATIOS) as ThumbnailAspectRatio[]).map(
							(key) => (
								<Button
									key={key}
									size="sm"
			...
			<Section
				...
				footer="Auto-summarize is enabled. Conversation will be compacted when needed."
			>
				<Card className="bg-content2">
					<CardBody className="p-4 space-y-4">
						<div className="space-y-2">
							...
							<div className="flex justify-between text-tiny text-default-500">
								...
								<span>
									Available: {formatTokens(USABLE_LIMIT - promptTokens)}
								</span>
							</div>
						</div>
						<div className="grid grid-cols-4 gap-2 text-center">
							...
							<StatItem
								label="Available"
								value={formatTokens(USABLE_LIMIT - promptTokens)}
							/>
							<StatItem
								label="Buffer"
								value={formatTokens(BUFFER_SIZE)}
				...
				{!showDeleteConfirm ? (
					...
					<Card className="bg-danger/10 border border-danger/30">
						<CardBody className="p-4">
							<p className="text-small text-danger mb-3">
								This will permanently delete your project. This action cannot be
								undone.
							</p>
							<div className="flex gap-2">
								<Button
	...
	return (
		<div className="space-y-2">
			<div className="flex items-center gap-2 text-default-500">
				...
				<span className="text-small font-medium uppercase tracking-wide">
					{title}
				</span>
			</div>
			{children}
